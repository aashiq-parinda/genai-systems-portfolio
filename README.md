...
> * 📐 **[Complete GPT Mathematical Formulas Explained](GPT_MATHEMATICAL_FORMULAS_EXPLAINED.md)**: Every mathematical formula in the entire GPT architecture (from Token & Positional Embeddings, LayerNorm/RMSNorm, Causal Multi-Head Attention, GELU/SwiGLU, to AdamW, Sampling, and Scaling Laws) provided in **LaTeX Math, Word Form Equations, and Intuitive Layman Explanations**!
> * 🔬 **[GPT Mathematical Playground From Scratch Notebook](./notebooks/GPT_Mathematical_Playground_From_Scratch.ipynb)**: Executable, zero-dependency pure NumPy companion notebook implementing all 9 formula phases from scratch, featuring live variance proofs, Softmax overflow tests, RMSNorm speed benchmarks, and capacity calculators!
...
> * 🏢 **[AI Agents: Building an AI-Powered Software Company Masterclass Notebook](./notebooks/AI_Agents_Software_Company.ipynb)**: Complete production-grade hands-on simulation of an AI software company (NovaStack / DevFlow) across 25 parts: Reason-Act-Observe loops, Pydantic tools, LangGraph state machines, HITL safety gates, multi-agent supervisor topologies, Model Context Protocol (MCP), Agentic RAG, telemetry, and failure engineering!
> * 🐍 **[Python Enough for GenAI Masterclass Notebook](./notebooks/Python_Enough_for_GenAI.ipynb)**: The zero-fluff, production-grade Python curriculum across 16 modules engineered strictly for GenAI Systems: Foundations, Object Model, Closures & Decorators, Data Handling, Generators & Memory, Defensive Exceptions, OOP & Typing, Pydantic v2, HTTP/httpx, Async & Concurrency, FastAPI Microservices, Production Resilience (Rate Limiting, Idempotency, Caching), RAG, Autonomous ReAct Agents, and an end-to-end Enterprise Capstone!
> * 🧠 **[Complete Python → GenAI Curriculum Notebook](./notebooks/Complete_Python_GenAI_Curriculum.ipynb)**: The definitive 7-tier master curriculum spanning Core Python, Python Internals (`==` vs `is`, GIL, GC, memory), Advanced Python (Generators, Context Managers, OOP), Concurrency (Threads vs Asyncio vs Multiprocessing), Backend (FastAPI, Token Bucket, Idempotency), GenAI (Streaming, Structured Outputs, Vector Math, RAG, ReAct Agents), and Production Systems (Logging, Metrics, Circuit Breaker, Profiling, CAP)!

---

## 📺 Systems Architecture & Live Demo Walkthrough

[![Watch the Systems Portfolio Walkthrough](https://img.youtube.com/vi/2vUcTsUDV38/maxresdefault.jpg)](https://youtu.be/2vUcTsUDV38)
...
| **[🔬 GPT Mathematical Playground From Scratch](./notebooks/GPT_Mathematical_Playground_From_Scratch.ipynb)** | **Formula-by-Formula Pure Math Implementation** | Pure Python + NumPy | • **Embeddings & Pos**: Lookup, Sinusoidal Wavelengths, Learned PE, RoPE.<br>• **Norms & Attention**: LayerNorm vs RMSNorm benchmark, $\frac{1}{\sqrt{d_k}}$ Variance Proof, Numerically Stable Softmax ($z - \max(z)$), MHA, KV-Cache.<br>• **FFN & Activations**: GELU exact vs approx, SwiGLU gated FFN, Residual Highway.<br>• **Training Dynamics**: AdamW from scratch, Cosine Warmup, Gradient Clipping.<br>• **Capacity Calculators**: Total parameters ($12Ld^2$), FLOPs ($6ND$), KV-cache VRAM, Chinchilla scaling. |
| **[🏢 AI Agents: Software Company Masterclass](./notebooks/AI_Agents_Software_Company.ipynb)** | **Reasoning Loops, LangGraph, MCP, Multi-Agent & RAG** | Python, Pydantic v2, LangGraph, NetworkX | • **Dual-Mode Execution**: Frontier API or deterministic zero-cost simulator.<br>• **Tool Calling**: Pydantic v2 schemas, semantic affordances, runtime interception.<br>• **Orchestration**: Custom ReAct loop & LangGraph StateGraph with cyclic revisions.<br>• **Guardrails**: HITL production deployment gate & circuit breakers.<br>• **Multi-Agent**: Specialized workforce (Supervisor, Researcher, Architect, Dev, Reviewer, QA).<br>• **Protocol & RAG**: Local MCP JSON-RPC server & Agentic self-reflective RAG.<br>• **Production Capstone**: Full DevFlow launch proposal with telemetry & cost charts. |
| **[🐍 Python Enough for GenAI](./notebooks/Python_Enough_for_GenAI.ipynb)** | **Python Foundations to Enterprise GenAI Engineering** | Python 3.10+, FastAPI, Pydantic v2, httpx, asyncio | • **Zero-Fluff Foundations**: Chat messages, context slicing, object model, mutability trap.<br>• **Modern Tooling**: Closures, `@retry` with jitter, JSON fences, lazy generators.<br>• **Architecture**: Abstract `ModelProvider`, Protocols, Pydantic v2 schemas, async concurrency (20x benchmark).<br>• **Production & Capstone**: Token Bucket, Idempotency keys, RAG pipeline, ReAct agent loop, and complete Mini Enterprise GenAI Assistant microservice. |
| **[🧠 Complete Python → GenAI Curriculum](./notebooks/Complete_Python_GenAI_Curriculum.ipynb)** | **Foundational Python to Distributed GenAI Systems** | Python 3.10+, FastAPI, Pydantic v2, asyncio, numpy | • **Core & Internals**: Object identity, memory address interning, GIL bypass, GC cyclic reference breaker.<br>• **Advanced & Concurrency**: Custom `@retry` with jitter, pipeline generators, asyncio vs threads vs multiprocessing.<br>• **Backend & GenAI**: FastAPI router with DI, Token Bucket rate limiter, SSE streaming, RAG from scratch, ReAct loop.<br>• **Production & Capstone**: Structured JSON logging, SLA latency percentiles (p50/p95/p99), Circuit Breaker, and Grand Capstone integration. |
